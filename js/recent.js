...
function setupEventListeners() {
    ...
    gridViewBtn.addEventListener('click', () => {
        recentContainer.className = 'files-grid';
        gridViewBtn.classList.add('active');
        listViewBtn.classList.remove('active');
    });

    listViewBtn.addEventListener('click', () => {
        recentContainer.className = 'files-list';
...
function loadRecentFiles() {
    // Get recent files (files accessed in the last 30 days)
    const thirtyDaysAgo = Date.now() - (30 * 24 * 60 * 60 * 1000);
    const recentFiles = files.filter(file => {
        const lastAccessed = parseInt(localStorage.getItem(`file_${file.id}_lastAccessed`) || '0');
        return lastAccessed > thirtyDaysAgo && file.userId === currentUser.username;
    }).sort((a, b) => {
        const aTime = parseInt(localStorage.getItem(`file_${a.id}_lastAccessed`) || '0');
        const bTime = parseInt(localStorage.getItem(`file_${b.id}_lastAccessed`) || '0');
        return bTime - aTime;
    });
    ...
    if (recentFiles.length > 0) {
        ...
        recentFiles.forEach(file => displayFile(file));
    } else {
...
function displayFile(file) {
    ...
    recentContainer.appendChild(fileElement);
}
